...
func (as SqlOAuthStore) SaveApp(app *model.OAuthApp) StoreChannel {
	...
	go func() {
		...
		if len(app.Id) > 0 {
			result.Err = model.NewLocAppError("SqlOAuthStore.SaveApp", "i18n.server.store.sql_oauth.save_app.existing.app_error", nil, "app_id="+app.Id)
			storeChannel <- result
			close(storeChannel)
			return
		}
		...
		if err := as.GetMaster().Insert(app); err != nil {
			result.Err = model.NewLocAppError("SqlOAuthStore.SaveApp", "i18n.server.store.sql_oauth.save_app.save.app_error", nil, "app_id="+app.Id+", "+err.Error())
		} else {
			result.Data = app
		}
...
func (as SqlOAuthStore) UpdateApp(app *model.OAuthApp) StoreChannel {
	...
	go func() {
		...
		if oldAppResult, err := as.GetMaster().Get(model.OAuthApp{}, app.Id); err != nil {
			result.Err = model.NewLocAppError("SqlOAuthStore.UpdateApp", "i18n.server.store.sql_oauth.update_app.finding.app_error", nil, "app_id="+app.Id+", "+err.Error())
		} else if oldAppResult == nil {
			result.Err = model.NewLocAppError("SqlOAuthStore.UpdateApp", "i18n.server.store.sql_oauth.update_app.find.app_error", nil, "app_id="+app.Id)
		} else {
			...
			if count, err := as.GetMaster().Update(app); err != nil {
				result.Err = model.NewLocAppError("SqlOAuthStore.UpdateApp", "i18n.server.store.sql_oauth.update_app.updating.app_error", nil, "app_id="+app.Id+", "+err.Error())
			} else if count != 1 {
				result.Err = model.NewLocAppError("SqlOAuthStore.UpdateApp", "i18n.server.store.sql_oauth.update_app.update.app_error", nil, "app_id="+app.Id)
			} else {
...
func (as SqlOAuthStore) GetApp(id string) StoreChannel {
	...
	go func() {
		...
		if obj, err := as.GetReplica().Get(model.OAuthApp{}, id); err != nil {
			result.Err = model.NewAppError("SqlOAuthStore.GetApp", "i18n.server.store.sql_oauth.get_app.finding.app_error", nil, "app_id="+id+", "+err.Error(), http.StatusInternalServerError)
		} else if obj == nil {
			result.Err = model.NewAppError("SqlOAuthStore.GetApp", "i18n.server.store.sql_oauth.get_app.find.app_error", nil, "app_id="+id, http.StatusNotFound)
		} else {
...
func (as SqlOAuthStore) GetAppByUser(userId string, offset, limit int) StoreChannel {
	...
	go func() {
		...
		if _, err := as.GetReplica().Select(&apps, "SELECT * FROM OAuthApps WHERE CreatorId = :UserId LIMIT :Limit OFFSET :Offset", map[string]interface{}{"UserId": userId, "Offset": offset, "Limit": limit}); err != nil {
			result.Err = model.NewAppError("SqlOAuthStore.GetAppByUser", "i18n.server.store.sql_oauth.get_app_by_user.find.app_error", nil, "user_id="+userId+", "+err.Error(), http.StatusInternalServerError)
		}
...
func (as SqlOAuthStore) GetApps(offset, limit int) StoreChannel {
	...
	go func() {
		...
		if _, err := as.GetReplica().Select(&apps, "SELECT * FROM OAuthApps LIMIT :Limit OFFSET :Offset", map[string]interface{}{"Offset": offset, "Limit": limit}); err != nil {
			result.Err = model.NewAppError("SqlOAuthStore.GetAppByUser", "i18n.server.store.sql_oauth.get_apps.find.app_error", nil, "err="+err.Error(), http.StatusInternalServerError)
		}
...
func (as SqlOAuthStore) GetAuthorizedApps(userId string, offset, limit int) StoreChannel {
	...
	go func() {
		...
		if _, err := as.GetReplica().Select(&apps,
			...
			result.Err = model.NewAppError("SqlOAuthStore.GetAuthorizedApps", "i18n.server.store.sql_oauth.get_apps.find.app_error", nil, "err="+err.Error(), http.StatusInternalServerError)
		}
...
func (as SqlOAuthStore) DeleteApp(id string) StoreChannel {
	...
	go func() {
		...
		if err != nil {
			result.Err = model.NewLocAppError("SqlOAuthStore.DeleteApp", "i18n.server.store.sql_oauth.delete.open_transaction.app_error", nil, err.Error())
		} else {
			...
			if result.Err == nil {
				if err := transaction.Commit(); err != nil {
					...
					result.Err = model.NewLocAppError("SqlOAuthStore.DeleteApp", "i18n.server.store.sql_oauth.delete.commit_transaction.app_error", nil, err.Error())
				}
			} else {
				if err := transaction.Rollback(); err != nil {
					result.Err = model.NewLocAppError("SqlOAuthStore.DeleteApp", "i18n.server.store.sql_oauth.delete.rollback_transaction.app_error", nil, err.Error())
				}
			}
		}
...
func (as SqlOAuthStore) SaveAccessData(accessData *model.AccessData) StoreChannel {
	...
	go func() {
		...
		if err := as.GetMaster().Insert(accessData); err != nil {
			result.Err = model.NewLocAppError("SqlOAuthStore.SaveAccessData", "i18n.server.store.sql_oauth.save_access_data.app_error", nil, err.Error())
		} else {
...
func (as SqlOAuthStore) GetAccessData(token string) StoreChannel {
	...
	go func() {
		...
		if err := as.GetReplica().SelectOne(&accessData, "SELECT * FROM OAuthAccessData WHERE Token = :Token", map[string]interface{}{"Token": token}); err != nil {
			result.Err = model.NewLocAppError("SqlOAuthStore.GetAccessData", "i18n.server.store.sql_oauth.get_access_data.app_error", nil, err.Error())
		} else {
			result.Data = &accessData
		}
	...
	go func() {
		...
		if _, err := as.GetReplica().Select(&accessData,
			...
			result.Err = model.NewLocAppError("SqlOAuthStore.GetAccessDataByUserForApp",
				"i18n.server.store.sql_oauth.get_access_data_by_user_for_app.app_error", nil,
	...
	go func() {
		...
		if err := as.GetReplica().SelectOne(&accessData, "SELECT * FROM OAuthAccessData WHERE ClientId = :ClientId AND UserId = :UserId",
			...
			if strings.Contains(err.Error(), "no rows") {
				...
				result.Err = model.NewLocAppError("SqlOAuthStore.GetPreviousAccessData", "i18n.server.store.sql_oauth.get_previous_access_data.app_error", nil, err.Error())
			}
		} else {
			result.Data = &accessData
		}
...
func (as SqlOAuthStore) UpdateAccessData(accessData *model.AccessData) StoreChannel {
	...
	go func() {
		...
		if _, err := as.GetMaster().Exec("UPDATE OAuthAccessData SET Token = :Token, ExpiresAt = :ExpiresAt WHERE ClientId = :ClientId AND UserID = :UserId",
			...
			result.Err = model.NewLocAppError("SqlOAuthStore.Update", "i18n.server.store.sql_oauth.update_access_data.app_error", nil,
...
func (as SqlOAuthStore) RemoveAccessData(token string) StoreChannel {
	...
	go func() {
		...
		if _, err := as.GetMaster().Exec("DELETE FROM OAuthAccessData WHERE Token = :Token", map[string]interface{}{"Token": token}); err != nil {
			result.Err = model.NewLocAppError("SqlOAuthStore.RemoveAccessData", "i18n.server.store.sql_oauth.remove_access_data.app_error", nil, "err="+err.Error())
		}
...
func (as SqlOAuthStore) SaveAuthData(authData *model.AuthData) StoreChannel {
	...
	go func() {
		...
		if err := as.GetMaster().Insert(authData); err != nil {
			result.Err = model.NewLocAppError("SqlOAuthStore.SaveAuthData", "i18n.server.store.sql_oauth.save_auth_data.app_error", nil, err.Error())
		} else {
...
func (as SqlOAuthStore) GetAuthData(code string) StoreChannel {
	...
	go func() {
		...
		if obj, err := as.GetReplica().Get(model.AuthData{}, code); err != nil {
			result.Err = model.NewLocAppError("SqlOAuthStore.GetAuthData", "i18n.server.store.sql_oauth.get_auth_data.finding.app_error", nil, err.Error())
		} else if obj == nil {
			result.Err = model.NewLocAppError("SqlOAuthStore.GetAuthData", "i18n.server.store.sql_oauth.get_auth_data.find.app_error", nil, "")
		} else {
...
func (as SqlOAuthStore) RemoveAuthData(code string) StoreChannel {
	...
	go func() {
		...
		if err != nil {
			result.Err = model.NewLocAppError("SqlOAuthStore.RemoveAuthData", "i18n.server.store.sql_oauth.remove_auth_data.app_error", nil, "err="+err.Error())
		}
...
func (as SqlOAuthStore) PermanentDeleteAuthDataByUser(userId string) StoreChannel {
	...
	go func() {
		...
		if err != nil {
			result.Err = model.NewLocAppError("SqlOAuthStore.RemoveAuthDataByUserId", "i18n.server.store.sql_oauth.permanent_delete_auth_data_by_user.app_error", nil, "err="+err.Error())
		}
...
func (as SqlOAuthStore) deleteApp(transaction *gorp.Transaction, clientId string) StoreResult {
	...
	if _, err := transaction.Exec("DELETE FROM OAuthApps WHERE Id = :Id", map[string]interface{}{"Id": clientId}); err != nil {
		result.Err = model.NewLocAppError("SqlOAuthStore.DeleteApp", "i18n.server.store.sql_oauth.delete_app.app_error", nil, "id="+clientId+", err="+err.Error())
		return result
	}
...
func (as SqlOAuthStore) deleteAppExtras(transaction *gorp.Transaction, clientId string) StoreResult {
	...
	if _, err := transaction.Exec(
		...
		result.Err = model.NewLocAppError("SqlOAuthStore.DeleteApp", "i18n.server.store.sql_preference.delete.app_error", nil, err.Error())
		return result
	}
