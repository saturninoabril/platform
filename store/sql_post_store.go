	...
	go func() {
		...
		if len(post.Id) > 0 {
			result.Err = model.NewLocAppError("SqlPostStore.Save",
				"i18n.server.store.sql_post.save.existing.app_error", nil, "id="+post.Id)
			storeChannel <- result
			close(storeChannel)
			return
		}
		...
		if err := s.GetMaster().Insert(post); err != nil {
			result.Err = model.NewLocAppError("SqlPostStore.Save", "i18n.server.store.sql_post.save.app_error", nil, "id="+post.Id+", "+err.Error())
		} else {
			time := post.UpdateAt
...
func (s SqlPostStore) Update(newPost *model.Post, oldPost *model.Post) StoreChannel {
	...
	go func() {
		...
		if _, err := s.GetMaster().Update(newPost); err != nil {
			result.Err = model.NewLocAppError("SqlPostStore.Update", "i18n.server.store.sql_post.update.app_error", nil, "id="+newPost.Id+", "+err.Error())
		} else {
...
func (s SqlPostStore) Overwrite(post *model.Post) StoreChannel {
	...
	go func() {
		...
		if _, err := s.GetMaster().Update(post); err != nil {
			result.Err = model.NewLocAppError("SqlPostStore.Overwrite", "i18n.server.store.sql_post.overwrite.app_error", nil, "id="+post.Id+", "+err.Error())
		} else {
			result.Data = post
		}
...
func (s SqlPostStore) GetFlaggedPosts(userId string, offset int, limit int) StoreChannel {
	...
	go func() {
		...
		if _, err := s.GetReplica().Select(&posts, "SELECT * FROM Posts WHERE Id IN (SELECT Name FROM Preferences WHERE UserId = :UserId AND Category = :Category) AND DeleteAt = 0 ORDER BY CreateAt DESC LIMIT :Limit OFFSET :Offset", map[string]interface{}{"UserId": userId, "Category": model.PREFERENCE_CATEGORY_FLAGGED_POST, "Offset": offset, "Limit": limit}); err != nil {
			result.Err = model.NewLocAppError("SqlPostStore.GetFlaggedPosts", "i18n.server.store.sql_post.get_flagged_posts.app_error", nil, err.Error())
		} else {
...
func (s SqlPostStore) GetFlaggedPostsForTeam(userId, teamId string, offset int, limit int) StoreChannel {
	...
	go func() {
		...
		if _, err := s.GetReplica().Select(&posts, query, map[string]interface{}{"UserId": userId, "Category": model.PREFERENCE_CATEGORY_FLAGGED_POST, "Offset": offset, "Limit": limit, "TeamId": teamId}); err != nil {
			result.Err = model.NewLocAppError("SqlPostStore.GetFlaggedPostsForTeam", "i18n.server.store.sql_post.get_flagged_posts.app_error", nil, err.Error())
		} else {
			for _, post := range posts {
				pl.AddPost(post)
				pl.AddOrder(post.Id)
...
func (s SqlPostStore) GetFlaggedPostsForChannel(userId, channelId string, offset int, limit int) StoreChannel {
	...
	go func() {
		...
		if _, err := s.GetReplica().Select(&posts, query, map[string]interface{}{"UserId": userId, "Category": model.PREFERENCE_CATEGORY_FLAGGED_POST, "ChannelId": channelId, "Offset": offset, "Limit": limit}); err != nil {
			result.Err = model.NewLocAppError("SqlPostStore.GetFlaggedPostsForChannel", "i18n.server.store.sql_post.get_flagged_posts.app_error", nil, err.Error())
		} else {
			for _, post := range posts {
				pl.AddPost(post)
				pl.AddOrder(post.Id)
...
func (s SqlPostStore) Get(id string) StoreChannel {
	...
	go func() {
		...
		if len(id) == 0 {
			result.Err = model.NewLocAppError("SqlPostStore.GetPost", "i18n.server.store.sql_post.get.app_error", nil, "id="+id)
			storeChannel <- result
			close(storeChannel)
			return
		}
		...
		if err != nil {
			result.Err = model.NewLocAppError("SqlPostStore.GetPost", "i18n.server.store.sql_post.get.app_error", nil, "id="+id+err.Error())
			storeChannel <- result
			close(storeChannel)
			return
		}
		...
		if len(rootId) == 0 {
			result.Err = model.NewLocAppError("SqlPostStore.GetPost", "i18n.server.store.sql_post.get.app_error", nil, "root_id="+rootId)
			storeChannel <- result
			close(storeChannel)
			return
		}
		...
		if err != nil {
			result.Err = model.NewLocAppError("SqlPostStore.GetPost", "i18n.server.store.sql_post.get.app_error", nil, "root_id="+rootId+err.Error())
			storeChannel <- result
			close(storeChannel)
			return
		} else {
...
func (s SqlPostStore) GetSingle(id string) StoreChannel {
	...
	go func() {
		...
		err := s.GetReplica().SelectOne(&post, "SELECT * FROM Posts WHERE Id = :Id AND DeleteAt = 0", map[string]interface{}{"Id": id})
		if err != nil {
			result.Err = model.NewLocAppError("SqlPostStore.GetSingle", "i18n.server.store.sql_post.get.app_error", nil, "id="+id+err.Error())
		}
...
func (s SqlPostStore) Delete(postId string, time int64) StoreChannel {
	...
	go func() {
		...
		if err != nil {
			result.Err = model.NewLocAppError("SqlPostStore.Delete", "i18n.server.store.sql_post.delete.app_error", nil, "id="+postId+", err="+err.Error())
		}
...
func (s SqlPostStore) permanentDelete(postId string) StoreChannel {
	...
	go func() {
		...
		if err != nil {
			result.Err = model.NewLocAppError("SqlPostStore.Delete", "i18n.server.store.sql_post.permanent_delete.app_error", nil, "id="+postId+", err="+err.Error())
		}
...
func (s SqlPostStore) permanentDeleteAllCommentByUser(userId string) StoreChannel {
	...
	go func() {
		...
		if err != nil {
			result.Err = model.NewLocAppError("SqlPostStore.permanentDeleteAllCommentByUser", "i18n.server.store.sql_post.permanent_delete_all_comments_by_user.app_error", nil, "userId="+userId+", err="+err.Error())
		}
...
func (s SqlPostStore) PermanentDeleteByUser(userId string) StoreChannel {
	...
	go func() {
		...
		for found {
			...
			if err != nil {
				result.Err = model.NewLocAppError("SqlPostStore.PermanentDeleteByUser.select", "i18n.server.store.sql_post.permanent_delete_by_user.app_error", nil, "userId="+userId+", err="+err.Error())
				storeChannel <- result
				close(storeChannel)
				return
			} else {
			...
			count = count + 1
			if count >= 10 {
				result.Err = model.NewLocAppError("SqlPostStore.PermanentDeleteByUser.toolarge", "i18n.server.store.sql_post.permanent_delete_by_user.too_many.app_error", nil, "userId="+userId)
				storeChannel <- result
				close(storeChannel)
				return
			}
...
func (s SqlPostStore) PermanentDeleteByChannel(channelId string) StoreChannel {
	...
	go func() {
		...
		if _, err := s.GetMaster().Exec("DELETE FROM Posts WHERE ChannelId = :ChannelId", map[string]interface{}{"ChannelId": channelId}); err != nil {
			result.Err = model.NewLocAppError("SqlPostStore.PermanentDeleteByChannel", "i18n.server.store.sql_post.permanent_delete_by_channel.app_error", nil, "channel_id="+channelId+", "+err.Error())
		}
...
func (s SqlPostStore) GetPosts(channelId string, offset int, limit int, allowFromCache bool) StoreChannel {
	...
	go func() {
		...
		if limit > 1000 {
			result.Err = model.NewLocAppError("SqlPostStore.GetLinearPosts", "i18n.server.store.sql_post.get_posts.app_error", nil, "channelId="+channelId)
			storeChannel <- result
			close(storeChannel)
			return
		}
...
func (s SqlPostStore) GetPostsSince(channelId string, time int64, allowFromCache bool) StoreChannel {
	...
	go func() {
		...
		if err != nil {
			result.Err = model.NewLocAppError("SqlPostStore.GetPostsSince", "i18n.server.store.sql_post.get_posts_since.app_error", nil, "channelId="+channelId+err.Error())
		} else {
...
func (s SqlPostStore) getPostsAround(channelId string, postId string, numPosts int, offset int, before bool) StoreChannel {
	...
	go func() {
		...
		if err1 != nil {
			result.Err = model.NewLocAppError("SqlPostStore.GetPostContext", "i18n.server.store.sql_post.get_posts_around.get.app_error", nil, "channelId="+channelId+err1.Error())
		} else if err2 != nil {
			result.Err = model.NewLocAppError("SqlPostStore.GetPostContext", "i18n.server.store.sql_post.get_posts_around.get_parent.app_error", nil, "channelId="+channelId+err2.Error())
		} else {
...
func (s SqlPostStore) getRootPosts(channelId string, offset int, limit int) StoreChannel {
	...
	go func() {
		...
		if err != nil {
			result.Err = model.NewLocAppError("SqlPostStore.GetLinearPosts", "i18n.server.store.sql_post.get_root_posts.app_error", nil, "channelId="+channelId+err.Error())
		} else {
			result.Data = posts
		}
...
func (s SqlPostStore) getParentsPosts(channelId string, offset int, limit int) StoreChannel {
	...
	go func() {
		...
		if err != nil {
			result.Err = model.NewLocAppError("SqlPostStore.GetLinearPosts", "i18n.server.store.sql_post.get_parents_posts.app_error", nil, "channelId="+channelId+err.Error())
		} else {
			result.Data = posts
		}
...
func (s SqlPostStore) Search(teamId string, userId string, params *model.SearchParams) StoreChannel {
	...
	go func() {
		...
		if !*utils.Cfg.ServiceSettings.EnablePostSearch {
			...
			result.Err = model.NewLocAppError("SqlPostStore.Search", "i18n.server.store.sql_post.search.disabled", nil, fmt.Sprintf("teamId=%v userId=%v params=%v", teamId, userId, params.ToJson()))
			storeChannel <- result
			close(storeChannel)
			return
		}
		...
		if err != nil {
			l4g.Warn(utils.T("i18n.server.store.sql_post.search.warn"), err.Error())
...
func (s SqlPostStore) AnalyticsUserCountsWithPostsByDay(teamId string) StoreChannel {
	...
	go func() {
		...
		if err != nil {
			result.Err = model.NewLocAppError("SqlPostStore.AnalyticsUserCountsWithPostsByDay", "i18n.server.store.sql_post.analytics_user_counts_posts_by_day.app_error", nil, err.Error())
		} else {
			result.Data = rows
		}
...
func (s SqlPostStore) AnalyticsPostCountsByDay(teamId string) StoreChannel {
	...
	go func() {
		...
		_, err := s.GetReplica().Select(
			...
			map[string]interface{}{"TeamId": teamId, "StartTime": start, "EndTime": end})
		if err != nil {
			result.Err = model.NewLocAppError("SqlPostStore.AnalyticsPostCountsByDay", "i18n.server.store.sql_post.analytics_posts_count_by_day.app_error", nil, err.Error())
		} else {
			result.Data = rows
		}
...
func (s SqlPostStore) AnalyticsPostCount(teamId string, mustHaveFile bool, mustHaveHashtag bool) StoreChannel {
	...
	go func() {
		...
		if v, err := s.GetReplica().SelectInt(query, map[string]interface{}{"TeamId": teamId}); err != nil {
			result.Err = model.NewLocAppError("SqlPostStore.AnalyticsPostCount", "i18n.server.store.sql_post.analytics_posts_count.app_error", nil, err.Error())
		} else {
			result.Data = v
		}
...
func (s SqlPostStore) GetPostsCreatedAt(channelId string, time int64) StoreChannel {
	...
	go func() {
		...
		if err != nil {
			result.Err = model.NewAppError("SqlPostStore.GetPostsCreatedAt", "i18n.server.store.sql_post.get_posts_created_att.app_error", nil, "channelId="+channelId+err.Error(), http.StatusInternalServerError)
		} else {
			result.Data = posts
		}
