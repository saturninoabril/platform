...
const (
	MISSING_ACCOUNT_ERROR                      = "i18n.server.store.sql_user.missing_account.const"
	MISSING_AUTH_ACCOUNT_ERROR                 = "i18n.server.store.sql_user.get_by_auth.missing_account.app_error"
...
func (us SqlUserStore) Save(user *model.User) StoreChannel {
	...
	go func() {
		...
		if len(user.Id) > 0 {
			result.Err = model.NewLocAppError("SqlUserStore.Save", "i18n.server.store.sql_user.save.existing.app_error", nil, "user_id="+user.Id)
			storeChannel <- result
			close(storeChannel)
			return
		}
		...
		if err := us.GetMaster().Insert(user); err != nil {
			if IsUniqueConstraintError(err.Error(), []string{"Email", "users_email_key", "idx_users_email_unique"}) {
				result.Err = model.NewAppError("SqlUserStore.Save", "i18n.server.store.sql_user.save.email_exists.app_error", nil, "user_id="+user.Id+", "+err.Error(), http.StatusBadRequest)
			} else if IsUniqueConstraintError(err.Error(), []string{"Username", "users_username_key", "idx_users_username_unique"}) {
				result.Err = model.NewAppError("SqlUserStore.Save", "i18n.server.store.sql_user.save.username_exists.app_error", nil, "user_id="+user.Id+", "+err.Error(), http.StatusBadRequest)
			} else {
				result.Err = model.NewLocAppError("SqlUserStore.Save", "i18n.server.store.sql_user.save.app_error", nil, "user_id="+user.Id+", "+err.Error())
			}
		} else {
			result.Data = user
		}
...
func (us SqlUserStore) Update(user *model.User, trustedUpdateData bool) StoreChannel {
	...
	go func() {
		...
		if oldUserResult, err := us.GetMaster().Get(model.User{}, user.Id); err != nil {
			result.Err = model.NewLocAppError("SqlUserStore.Update", "i18n.server.store.sql_user.update.finding.app_error", nil, "user_id="+user.Id+", "+err.Error())
		} else if oldUserResult == nil {
			result.Err = model.NewLocAppError("SqlUserStore.Update", "i18n.server.store.sql_user.update.find.app_error", nil, "user_id="+user.Id)
		} else {
			...
			if user.IsOAuthUser() {
				...
				if user.Username != oldUser.Username ||
					user.Email != oldUser.Email {
					result.Err = model.NewLocAppError("SqlUserStore.Update", "i18n.server.store.sql_user.update.can_not_change_ldap.app_error", nil, "user_id="+user.Id)
					storeChannel <- result
					close(storeChannel)
					return
				}
			...
			if count, err := us.GetMaster().Update(user); err != nil {
				if IsUniqueConstraintError(err.Error(), []string{"Email", "users_email_key", "idx_users_email_unique"}) {
					result.Err = model.NewLocAppError("SqlUserStore.Update", "i18n.server.store.sql_user.update.email_taken.app_error", nil, "user_id="+user.Id+", "+err.Error())
				} else if IsUniqueConstraintError(err.Error(), []string{"Username", "users_username_key", "idx_users_username_unique"}) {
					result.Err = model.NewLocAppError("SqlUserStore.Update", "i18n.server.store.sql_user.update.username_taken.app_error", nil, "user_id="+user.Id+", "+err.Error())
				} else {
					result.Err = model.NewLocAppError("SqlUserStore.Update", "i18n.server.store.sql_user.update.updating.app_error", nil, "user_id="+user.Id+", "+err.Error())
				}
			} else if count != 1 {
				result.Err = model.NewLocAppError("SqlUserStore.Update", "i18n.server.store.sql_user.update.app_error", nil, fmt.Sprintf("user_id=%v, count=%v", user.Id, count))
			} else {
...
func (us SqlUserStore) UpdateLastPictureUpdate(userId string) StoreChannel {
	...
	go func() {
		...
		if _, err := us.GetMaster().Exec("UPDATE Users SET LastPictureUpdate = :Time, UpdateAt = :Time WHERE Id = :UserId", map[string]interface{}{"Time": curTime, "UserId": userId}); err != nil {
			result.Err = model.NewLocAppError("SqlUserStore.UpdateUpdateAt", "i18n.server.store.sql_user.update_last_picture_update.app_error", nil, "user_id="+userId)
		} else {
			result.Data = userId
		}
...
func (us SqlUserStore) UpdateUpdateAt(userId string) StoreChannel {
	...
	go func() {
		...
		if _, err := us.GetMaster().Exec("UPDATE Users SET UpdateAt = :Time WHERE Id = :UserId", map[string]interface{}{"Time": curTime, "UserId": userId}); err != nil {
			result.Err = model.NewLocAppError("SqlUserStore.UpdateUpdateAt", "i18n.server.store.sql_user.update_update.app_error", nil, "user_id="+userId)
		} else {
			result.Data = userId
		}
...
func (us SqlUserStore) UpdatePassword(userId, hashedPassword string) StoreChannel {
	...
	go func() {
		...
		if _, err := us.GetMaster().Exec("UPDATE Users SET Password = :Password, LastPasswordUpdate = :LastPasswordUpdate, UpdateAt = :UpdateAt, AuthData = NULL, AuthService = '', EmailVerified = true, FailedAttempts = 0 WHERE Id = :UserId", map[string]interface{}{"Password": hashedPassword, "LastPasswordUpdate": updateAt, "UpdateAt": updateAt, "UserId": userId}); err != nil {
			result.Err = model.NewLocAppError("SqlUserStore.UpdatePassword", "i18n.server.store.sql_user.update_password.app_error", nil, "id="+userId+", "+err.Error())
		} else {
			result.Data = userId
		}
...
func (us SqlUserStore) UpdateFailedPasswordAttempts(userId string, attempts int) StoreChannel {
	...
	go func() {
		...
		if _, err := us.GetMaster().Exec("UPDATE Users SET FailedAttempts = :FailedAttempts WHERE Id = :UserId", map[string]interface{}{"FailedAttempts": attempts, "UserId": userId}); err != nil {
			result.Err = model.NewLocAppError("SqlUserStore.UpdateFailedPasswordAttempts", "i18n.server.store.sql_user.update_failed_pwd_attempts.app_error", nil, "user_id="+userId)
		} else {
			result.Data = userId
		}
...
func (us SqlUserStore) UpdateAuthData(userId string, service string, authData *string, email string, resetMfa bool) StoreChannel {
	...
	go func() {
		...
		if _, err := us.GetMaster().Exec(query, map[string]interface{}{"LastPasswordUpdate": updateAt, "UpdateAt": updateAt, "UserId": userId, "AuthService": service, "AuthData": authData, "Email": email}); err != nil {
			if IsUniqueConstraintError(err.Error(), []string{"Email", "users_email_key", "idx_users_email_unique"}) {
				result.Err = model.NewLocAppError("SqlUserStore.UpdateAuthData", "i18n.server.store.sql_user.update_auth_data.email_exists.app_error", map[string]interface{}{"Service": service, "Email": email}, "user_id="+userId+", "+err.Error())
			} else {
				result.Err = model.NewLocAppError("SqlUserStore.UpdateAuthData", "i18n.server.store.sql_user.update_auth_data.app_error", nil, "id="+userId+", "+err.Error())
			}
		} else {
			result.Data = userId
		}
...
func (us SqlUserStore) UpdateMfaSecret(userId, secret string) StoreChannel {
	...
	go func() {
		...
		if _, err := us.GetMaster().Exec("UPDATE Users SET MfaSecret = :Secret, UpdateAt = :UpdateAt WHERE Id = :UserId", map[string]interface{}{"Secret": secret, "UpdateAt": updateAt, "UserId": userId}); err != nil {
			result.Err = model.NewLocAppError("SqlUserStore.UpdateMfaSecret", "i18n.server.store.sql_user.update_mfa_secret.app_error", nil, "id="+userId+", "+err.Error())
		} else {
			result.Data = userId
		}
...
func (us SqlUserStore) UpdateMfaActive(userId string, active bool) StoreChannel {
	...
	go func() {
		...
		if _, err := us.GetMaster().Exec("UPDATE Users SET MfaActive = :Active, UpdateAt = :UpdateAt WHERE Id = :UserId", map[string]interface{}{"Active": active, "UpdateAt": updateAt, "UserId": userId}); err != nil {
			result.Err = model.NewLocAppError("SqlUserStore.UpdateMfaActive", "i18n.server.store.sql_user.update_mfa_active.app_error", nil, "id="+userId+", "+err.Error())
		} else {
			result.Data = userId
		}
...
func (us SqlUserStore) Get(id string) StoreChannel {
	...
	go func() {
		...
		if obj, err := us.GetReplica().Get(model.User{}, id); err != nil {
			result.Err = model.NewLocAppError("SqlUserStore.Get", "i18n.server.store.sql_user.get.app_error", nil, "user_id="+id+", "+err.Error())
		} else if obj == nil {
...
func (us SqlUserStore) GetAll() StoreChannel {
	...
	go func() {
		...
		if _, err := us.GetReplica().Select(&data, "SELECT * FROM Users"); err != nil {
			result.Err = model.NewLocAppError("SqlUserStore.GetAll", "i18n.server.store.sql_user.get.app_error", nil, err.Error())
		}
...
func (us SqlUserStore) GetAllProfiles(offset int, limit int) StoreChannel {
	...
	go func() {
		...
		if _, err := us.GetReplica().Select(&users, "SELECT * FROM Users ORDER BY Username ASC LIMIT :Limit OFFSET :Offset", map[string]interface{}{"Offset": offset, "Limit": limit}); err != nil {
			result.Err = model.NewLocAppError("SqlUserStore.GetAllProfiles", "i18n.server.store.sql_user.get_profiles.app_error", nil, err.Error())
		} else {
...
func (us SqlUserStore) GetProfiles(teamId string, offset int, limit int) StoreChannel {
	...
	go func() {
		...
		if _, err := us.GetReplica().Select(&users, "SELECT Users.* FROM Users, TeamMembers WHERE TeamMembers.TeamId = :TeamId AND Users.Id = TeamMembers.UserId AND TeamMembers.DeleteAt = 0 ORDER BY Users.Username ASC LIMIT :Limit OFFSET :Offset", map[string]interface{}{"TeamId": teamId, "Offset": offset, "Limit": limit}); err != nil {
			result.Err = model.NewLocAppError("SqlUserStore.GetProfiles", "i18n.server.store.sql_user.get_profiles.app_error", nil, err.Error())
		} else {
...
func (us SqlUserStore) GetProfilesInChannel(channelId string, offset int, limit int) StoreChannel {
	...
	go func() {
		...
		if _, err := us.GetReplica().Select(&users, query, map[string]interface{}{"ChannelId": channelId, "Offset": offset, "Limit": limit}); err != nil {
			result.Err = model.NewLocAppError("SqlUserStore.GetProfilesInChannel", "i18n.server.store.sql_user.get_profiles.app_error", nil, err.Error())
		} else {
...
func (us SqlUserStore) GetAllProfilesInChannel(channelId string, allowFromCache bool) StoreChannel {
	...
	go func() {
		...
		if _, err := us.GetReplica().Select(&users, query, map[string]interface{}{"ChannelId": channelId}); err != nil {
			result.Err = model.NewLocAppError("SqlUserStore.GetAllProfilesInChannel", "i18n.server.store.sql_user.get_profiles.app_error", nil, err.Error())
		} else {
...
func (us SqlUserStore) GetProfilesNotInChannel(teamId string, channelId string, offset int, limit int) StoreChannel {
	...
	go func() {
		...
		if _, err := us.GetReplica().Select(&users, `
			...
			result.Err = model.NewLocAppError("SqlUserStore.GetProfilesNotInChannel", "i18n.server.store.sql_user.get_profiles.app_error", nil, err.Error())
		} else {
...
func (us SqlUserStore) GetProfilesWithoutTeam(offset int, limit int) StoreChannel {
	...
	go func() {
		...
		if _, err := us.GetReplica().Select(&users, query, map[string]interface{}{"Offset": offset, "Limit": limit}); err != nil {
			result.Err = model.NewLocAppError("SqlUserStore.GetProfilesWithoutTeam", "i18n.server.store.sql_user.get_profiles.app_error", nil, err.Error())
		} else {
...
func (us SqlUserStore) GetProfilesByUsernames(usernames []string, teamId string) StoreChannel {
	...
	go func() {
		...
		if _, err := us.GetReplica().Select(&users, `SELECT Users.* FROM Users INNER JOIN TeamMembers ON
			...
			result.Err = model.NewLocAppError("SqlUserStore.GetProfilesByUsernames", "i18n.server.store.sql_user.get_profiles.app_error", nil, err.Error())
		} else {
			userMap := make(map[string]*model.User)
...
func (us SqlUserStore) GetRecentlyActiveUsersForTeam(teamId string) StoreChannel {
	...
	go func() {
		...
		if _, err := us.GetReplica().Select(&users, `
			...
			result.Err = model.NewLocAppError("SqlUserStore.GetRecentlyActiveUsers", "i18n.server.store.sql_user.get_recently_active_users.app_error", nil, err.Error())
		} else {
...
func (us SqlUserStore) GetProfileByIds(userIds []string, allowFromCache bool) StoreChannel {
	...
	go func() {
		...
		if _, err := us.GetReplica().Select(&users, "SELECT * FROM Users WHERE Users.Id IN ("+idQuery+")", props); err != nil {
			result.Err = model.NewLocAppError("SqlUserStore.GetProfileByIds", "i18n.server.store.sql_user.get_profiles.app_error", nil, err.Error())
		} else {
...
func (us SqlUserStore) GetSystemAdminProfiles() StoreChannel {
	...
	go func() {
		...
		if _, err := us.GetReplica().Select(&users, "SELECT * FROM Users WHERE Roles LIKE :Roles", map[string]interface{}{"Roles": "%system_admin%"}); err != nil {
			result.Err = model.NewLocAppError("SqlUserStore.GetSystemAdminProfiles", "i18n.server.store.sql_user.get_sysadmin_profiles.app_error", nil, err.Error())
		} else {
	...
	go func() {
		...
		if err := us.GetReplica().SelectOne(&user, "SELECT * FROM Users WHERE AuthData = :AuthData AND AuthService = :AuthService", map[string]interface{}{"AuthData": authData, "AuthService": authService}); err != nil {
			if err == sql.ErrNoRows {
				...
				result.Err = model.NewLocAppError("SqlUserStore.GetByAuth", "i18n.server.store.sql_user.get_by_auth.other.app_error", nil, "authData="+*authData+", authService="+authService+", "+err.Error())
			}
		}
...
func (us SqlUserStore) GetAllUsingAuthService(authService string) StoreChannel {
	...
	go func() {
		...
		if _, err := us.GetReplica().Select(&data, "SELECT * FROM Users WHERE AuthService = :AuthService", map[string]interface{}{"AuthService": authService}); err != nil {
			result.Err = model.NewLocAppError("SqlUserStore.GetByAuth", "i18n.server.store.sql_user.get_by_auth.other.app_error", nil, "authService="+authService+", "+err.Error())
		}
...
func (us SqlUserStore) GetByUsername(username string) StoreChannel {
	...
	go func() {
		...
		if err := us.GetReplica().SelectOne(&user, "SELECT * FROM Users WHERE Username = :Username", map[string]interface{}{"Username": username}); err != nil {
			result.Err = model.NewLocAppError("SqlUserStore.GetByUsername", "i18n.server.store.sql_user.get_by_username.app_error",
				nil, err.Error())
		}
...
func (us SqlUserStore) GetForLogin(loginId string, allowSignInWithUsername, allowSignInWithEmail, ldapEnabled bool) StoreChannel {
	...
	go func() {
		...
		if _, err := us.GetReplica().Select(
			...
			params); err != nil {
			result.Err = model.NewLocAppError("SqlUserStore.GetForLogin", "i18n.server.store.sql_user.get_for_login.app_error", nil, err.Error())
		} else if len(users) == 1 {
			result.Data = users[0]
		} else if len(users) > 1 {
			result.Err = model.NewLocAppError("SqlUserStore.GetForLogin", "i18n.server.store.sql_user.get_for_login.multiple_users", nil, "")
		} else {
			result.Err = model.NewLocAppError("SqlUserStore.GetForLogin", "i18n.server.store.sql_user.get_for_login.app_error", nil, "")
		}
...
func (us SqlUserStore) VerifyEmail(userId string) StoreChannel {
	...
	go func() {
		...
		if _, err := us.GetMaster().Exec("UPDATE Users SET EmailVerified = true WHERE Id = :UserId", map[string]interface{}{"UserId": userId}); err != nil {
			result.Err = model.NewLocAppError("SqlUserStore.VerifyEmail", "i18n.server.store.sql_user.verify_email.app_error", nil, "userId="+userId+", "+err.Error())
		}
...
func (us SqlUserStore) GetTotalUsersCount() StoreChannel {
	...
	go func() {
		...
		if count, err := us.GetReplica().SelectInt("SELECT COUNT(Id) FROM Users"); err != nil {
			result.Err = model.NewLocAppError("SqlUserStore.GetTotalUsersCount", "i18n.server.store.sql_user.get_total_users_count.app_error", nil, err.Error())
		} else {
			result.Data = count
		}
...
func (us SqlUserStore) PermanentDelete(userId string) StoreChannel {
	...
	go func() {
		...
		if _, err := us.GetMaster().Exec("DELETE FROM Users WHERE Id = :UserId", map[string]interface{}{"UserId": userId}); err != nil {
			result.Err = model.NewLocAppError("SqlUserStore.PermanentDelete", "i18n.server.store.sql_user.permanent_delete.app_error", nil, "userId="+userId+", "+err.Error())
		}
...
func (us SqlUserStore) AnalyticsUniqueUserCount(teamId string) StoreChannel {
	...
	go func() {
		...
		if err != nil {
			result.Err = model.NewLocAppError("SqlUserStore.AnalyticsUniqueUserCount", "i18n.server.store.sql_user.analytics_unique_user_count.app_error", nil, err.Error())
		} else {
			result.Data = v
		}
...
func (us SqlUserStore) AnalyticsActiveCount(timePeriod int64) StoreChannel {
	...
	go func() {
		...
		if err != nil {
			result.Err = model.NewLocAppError("SqlUserStore.AnalyticsDailyActiveUsers", "i18n.server.store.sql_user.analytics_daily_active_users.app_error", nil, err.Error())
		} else {
			result.Data = v
		}
...
func (us SqlUserStore) GetUnreadCount(userId string) StoreChannel {
	...
	go func() {
		...
		INNER JOIN ChannelMembers cm
			...
			result.Err = model.NewLocAppError("SqlUserStore.GetMentionCount", "i18n.server.store.sql_user.get_unread_count.app_error", nil, err.Error())
		} else {
			result.Data = count
		}
...
func (us SqlUserStore) GetUnreadCountForChannel(userId string, channelId string) StoreChannel {
	...
	go func() {
		...
		if count, err := us.GetReplica().SelectInt("SELECT SUM(CASE WHEN c.Type = 'D' THEN (c.TotalMsgCount - cm.MsgCount) ELSE cm.MentionCount END) FROM Channels c INNER JOIN ChannelMembers cm ON c.Id = :ChannelId AND cm.ChannelId = :ChannelId AND cm.UserId = :UserId", map[string]interface{}{"ChannelId": channelId, "UserId": userId}); err != nil {
			result.Err = model.NewLocAppError("SqlUserStore.GetMentionCountForChannel", "i18n.server.store.sql_user.get_unread_count_for_channel.app_error", nil, err.Error())
		} else {
			result.Data = count
		}
...
func (us SqlUserStore) performSearch(searchQuery string, term string, options map[string]bool, parameters map[string]interface{}) StoreResult {
	...
	if _, err := us.GetReplica().Select(&users, searchQuery, parameters); err != nil {
		result.Err = model.NewLocAppError("SqlUserStore.Search", "i18n.server.store.sql_user.search.app_error", nil, "term="+term+", "+"search_type="+searchType+", "+err.Error())
	} else {
		for _, u := range users {
			u.Password = ""
			u.AuthData = new(string)
...
func (us SqlUserStore) AnalyticsGetInactiveUsersCount() StoreChannel {
	...
	go func() {
		...
		if count, err := us.GetReplica().SelectInt("SELECT COUNT(Id) FROM Users WHERE DeleteAt > 0"); err != nil {
			result.Err = model.NewLocAppError("SqlUserStore.AnalyticsGetInactiveUsersCount", "i18n.server.store.sql_user.analytics_get_inactive_users_count.app_error", nil, err.Error())
		} else {
			result.Data = count
		}
...
func (us SqlUserStore) AnalyticsGetSystemAdminCount() StoreChannel {
	...
	go func() {
		...
		if count, err := us.GetReplica().SelectInt("SELECT count(*) FROM Users WHERE Roles LIKE :Roles and DeleteAt = 0", map[string]interface{}{"Roles": "%system_admin%"}); err != nil {
			result.Err = model.NewLocAppError("SqlUserStore.AnalyticsGetSystemAdminCount", "i18n.server.store.sql_user.analytics_get_system_admin_count.app_error", nil, err.Error())
		} else {
			result.Data = count
		}
...
func (us SqlUserStore) GetProfilesNotInTeam(teamId string, offset int, limit int) StoreChannel {
	...
	go func() {
		...
		if _, err := us.GetReplica().Select(&users, `
			...
			result.Err = model.NewLocAppError("SqlUserStore.GetProfilesNotInTeam", "i18n.server.store.sql_user.get_profiles.app_error", nil, err.Error())
		} else {
