...
func updateUserActive(c *Context, w http.ResponseWriter, r *http.Request) {
	...
	if !isSelfDeactive && !app.SessionHasPermissionTo(c.Session, model.PERMISSION_MANAGE_SYSTEM) {
		c.Err = model.NewLocAppError("updateUserActive", "i18n.server.api.user.update_active.permissions.app_error", nil, "userId="+c.Params.UserId)
...
func updatePassword(c *Context, w http.ResponseWriter, r *http.Request) {
	...
	if c.Params.UserId == c.Session.UserId {
		...
		err = app.UpdatePasswordByUserIdSendEmail(c.Params.UserId, newPassword, c.T("i18n.server.api.user.reset_password.method"))
	} else {
		err = model.NewAppError("updatePassword", "i18n.server.api.user.update_password.context.app_error", nil, "", http.StatusForbidden)
	}
...
func verifyUserEmail(c *Context, w http.ResponseWriter, r *http.Request) {
	...
	c.Err = model.NewLocAppError("verifyUserEmail", "i18n.server.api.user.verify_email.bad_link.app_error", nil, "")
	c.Err.StatusCode = http.StatusBadRequest
	return
}
