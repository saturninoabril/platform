...
func (me *msgProvider) GetCommand(T goi18n.TranslateFunc) *model.Command {
	return &model.Command{
		Trigger:          CMD_MSG,
		AutoComplete:     true,
		AutoCompleteDesc: T("i18n.server.api.command_msg.desc"),
		AutoCompleteHint: T("i18n.server.api.command_msg.hint"),
		DisplayName:      T("i18n.server.api.command_msg.name"),
	}
}
...
func (me *msgProvider) DoCommand(args *model.CommandArgs, message string) *model.CommandResponse {
	...
	if result := <-Srv.Store.User().GetByUsername(targetUsername); result.Err != nil {
		...
		return &model.CommandResponse{Text: args.T("i18n.server.api.command_msg.missing.app_error"), ResponseType: model.COMMAND_RESPONSE_TYPE_EPHEMERAL}
	} else {
	...
	if channel := <-Srv.Store.Channel().GetByName(args.TeamId, channelName, true); channel.Err != nil {
		if channel.Err.Id == "i18n.server.store.sql_channel.get_by_name.missing.app_error" {
			if directChannel, err := CreateDirectChannel(args.UserId, userProfile.Id); err != nil {
				l4g.Error(err.Error())
				return &model.CommandResponse{Text: args.T("i18n.server.api.command_msg.dm_fail.app_error"), ResponseType: model.COMMAND_RESPONSE_TYPE_EPHEMERAL}
			} else {
	...
	if len(parsedMessage) > 0 {
		...
		if _, err := CreatePost(post, args.TeamId, true); err != nil {
			return &model.CommandResponse{Text: args.T("i18n.server.api.command_msg.fail.app_error"), ResponseType: model.COMMAND_RESPONSE_TYPE_EPHEMERAL}
		}
	}
